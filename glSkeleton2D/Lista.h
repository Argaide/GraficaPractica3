...
//-----------CUARDAR_LISTA------------------------------
   void guardarLista(ofstream& sal)
   {
      sal.write((char*)& Elem, sizeof(int)); //Guardamos el tamaño de la lista
      int i = 0;
      Nodo<T>* aux=primero;
      while(i<Elem)
      {
         //listaGuardarDato(sal,aux->info);
         aux=aux->sig;
         i++;
      }
   }
...
//-----------CARGAR_LISTA------------------------------
   void cargarLista(ifstream& Ent)
   {

        int cont;
        T t;

        Ent.read((char*)& cont, sizeof(int));
        for (int i=0; i<cont; i++){
                //listaCargarDato(Ent,t);
                ponElem(t);
        }
